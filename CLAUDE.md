...
`generateScenario(seed)` (`src/lib/market/scenario.ts`, built on `generator.ts` + `rng.ts`/seedrandom) deterministically produces the whole 180-candle chart and the hidden answer (`setupLabel`). Grading/validation always re-derives from `scenario_seed` — never from the cached columns below — via `rebuildPlan` / `validateExit` in `src/lib/rep/server-guard.ts`. **Any change to the generator, scenario construction, or RNG consumption order changes every stored rep's recomputed entry/R** — treat it as a data migration. To bias which setups appear, pick seeds (`pickSeedForMix` / `setupLabelForSeed`), never alter generation.

`reps.setup_label`/`reps.entry_price` (added in `20260917_reps_cached_scenario_facts.sql`) cache `deriveScenarioFacts(seed)`'s output at commit time — a read-path performance optimization only, so list/summary/gate endpoints don't regenerate 180 candles per row. Rows committed before that migration have both columns `null`; `serverRepToRep` (`lib/rep/api.ts`) falls back to regenerating from the seed whenever either is `null`/`undefined`. `setup_label` is the graded answer, so it gets the same pre-GRADED stripping as `exit_price`/`r_result` (see below) — never add a new raw-row response path without checking that stripping.
...
- `GET /api/reps/summary` — `/progress` for a logged-in user calls this instead of reducing the full `GET /api/reps` payload client-side. It fetches all `GRADED`/`REVEALED` rows once server-side and returns already-computed numbers (adherence, expectancy, setup accuracy, gate evaluation, weekly comparison, pace, equity curve, judgment×outcome matrix) by calling the exact same `lib/metrics/*`/`lib/gate/rules.ts` functions the client used to call — no calculation logic was duplicated or rewritten, only *where* it runs moved. Guests skip this (capped at 5 local reps, no server round trip needed) and still compute client-side via `summaryFromLocalReps` in the page.
- `GET /api/reps` also takes optional `limit`/`before`/`setup`/`grade`/`traded=1` query params, used by `/journal` for cursor-paginated, server-filtered browsing (`{ reps, hasMore }` response shape) plus a separate unbounded-but-filtered call for its "CSV 내보내기" button (a user-initiated action, not a page-load default). With no params it's the original plain-array, unfiltered, unbounded response — `useRepLogStore` and the migration flow are untouched.
...
- Immediate feedback: `lib/feedback/rules.ts` is a priority-ordered rule list; the first matching rule wins. Deliberately rule-based — no LLM calls (this is a spec principle; keep it that way). `lib/feedback/summary.ts`'s `computeHistorySummary(before, current, dailyGoal)` composes this with the before/after adherence and required-sample numbers shown as "이번으로 달라진 것" — it's isomorphic (no server/browser-only APIs) so both the grade/pass API routes and the client call the exact same function. Logged-in practice gets it precomputed in the grade/pass response (`feedback` field) so `ImmediateFeedback` never needs the user's full rep history just to render; guests (and the `ImmediateFeedback` component as a defensive fallback) compute it client-side from `logReps` instead.
- Daily goal / weekly comparison / pace ETA: `lib/metrics/progress.ts`.
- AI coach advice (`lib/ai/advisor.ts`, `POST /api/reps/[id]/advice`, `components/rep/ai-advice.tsx`): a separate, opt-in feature — the user clicks a button on the reveal screen to get a short Claude-generated coaching note. Distinct from the rule-based immediate feedback above, which stays untouched. Only available to logged-in users (guests have no server-side rep row to fetch); requires `rep.state` to be `GRADED`/`REVEALED`, same result-lock boundary as everywhere else. The server rebuilds facts from the DB row + `rebuildPlan`, never trusts client-supplied results. The response streams as NDJSON (`AdviceStreamEvent`: `delta` / `done` / `error`, one per line) so text appears as it's written: failures before the first token still come back as a JSON error with a 429/502/500 status, failures or a refusal after it arrive as an `error` event (the 200 is already sent). `streamAdvice` tries Opus 5 first and retries once with Sonnet 5 only if the refusal happens before any text was yielded (at most 2 model calls); a refusal after partial text was already sent does not retry.
- `/quiz` (`app/quiz/page.tsx`, `lib/quiz/`): a separate setup-discrimination drill, no plan/replay/grading — pick a label, see the answer immediately. Writes nothing to the server; score and the missed-question review queue live entirely in `localStorage` (`reps.quiz.v1`), so it never touches gate/stats calculations.
